...
export default function FollowsPage() {
  ...
  const handleFollowToggle = async (targetUserId: Id<"users">, isFollowing: boolean) => {
    if (isFollowing) {
  ...
  return (
    <div className="container mx-auto p-6 space-y-8 bg-background text-foreground">
      <h1 className="text-3xl font-bold text-primary">User Search and Follow Management</h1>
              ...
              <ul className="space-y-4">
                {searchResults.map(
                  (user) =>
                    user &&
                    user._id !== userId && (
                      <li key={user._id} className="flex justify-between items-center p-4 rounded-md bg-muted hover:bg-accent">
                        <span className="font-medium text-foreground">{user.name || "Unknown User"}</span>
                        <button
                          onClick={() => handleFollowToggle(user._id, following?.some((f) => f?._id === user._id) || false)}
                        ...
                        >
                          {following?.some((f) => f?._id === user._id) ? "Unfollow" : "Follow"}
                        </button>
                      </li>
                    )
                )}
      ...
      <div className="space-y-6">
        <h2 className="text-2xl font-semibold text-primary">Users You're Following</h2>
        <ul className="space-y-4">
          {following?.map(
            (followedUser) =>
              followedUser && (
                <li key={followedUser._id} className="flex justify-between items-center p-4 rounded-md bg-muted hover:bg-accent">
                  <span className="font-medium text-foreground">{followedUser.name || "Unknown User"}</span>
                  <button
